...
exclude_patterns = [
    ...
    "README.md",
    "licenses/*",
    # Collection mode: skip the local autodoc API tree and link to upstream.
    # This repo does not vendor the Isaac Lab Python sources, so autosummary
    # cannot import `isaaclab` and would abort the build (builder-inited).
    "source/api",
    "source/api/*",
]
...
html_static_path = ["source/_static/css", "source/_static/js"]
html_css_files = ["custom.css"]
html_js_files = ["color_mode_cycle.js"]
...
# Collection-mode build: sphinx-multiversion is disabled (this collection repo
# has no per-release git tags), so the sidebar omits the version switcher.
html_sidebars = {
